...
use wgpu::{BufferBindingType, ShaderStages, TextureView, TextureViewDimension};
...
impl GameClient {
	...
	pub fn init(&mut self, event_loop: &ActiveEventLoop) {
		...
		let (block_texture, block_sampler) =
			world::registries::block_textures().create_texture_sampler(&canvas, "block");
					...
					entries: vec![
						BindingEntryDescriptor {
							...
							item: BindingItem::TextureView(
								&TextureView::new(&block_texture, &TextureViewConfig {
									name: "block",
						...
						BindingEntryDescriptor {
							visibility: ShaderStages::FRAGMENT,
							item: BindingItem::Sampler(&block_sampler),
						}
					],
				},
			),
			...
			Binding::new(
				...
				&BindingDescriptor {
					...
					entries: vec![BindingEntryDescriptor {
						...
						item: BindingItem::Buffer(&camera.buffer, BufferBindingType::Uniform),
					}],
				},
			),
		);
