...
impl<T> CubicVec<T> {
	pub fn from_fn<F: Fn(U8Vec3) -> T>(side: u8, pos_to_item: F) -> Self {
		let n = side as usize;
		let vec = (0..n * n * n).map(|idx| {
			let pos = U8Vec3::new((idx % n) as u8, ((idx / n) % n) as u8, (idx / (n * n)) as u8);
			pos_to_item(pos)
		}).collect();
...
impl<T> Registry<T> {
	...
	pub fn register(&mut self, name: &str, item: T) {
		self.names.insert(name.to_string(), self.items.len() as Id);
		self.items.push(item);
	}
	
	#[inline]
	pub fn by_id(&self, id: Id) -> &T {
		self.items
			.get(id as usize)
	...
	pub fn by_name(&self, name: &str) -> &T {
		self.by_id(self.id_from_name(name))
	}
	
	#[inline]
	pub fn by_id_mut(&mut self, id: Id) -> &mut T {
		self.items
			.get_mut(id as usize)
			.expect(&format!("Item with id {} hasn't been registered", id))
	}
	...
	#[inline]
	pub fn by_name_mut(&mut self, name: &str) -> &mut T {
		self.by_id_mut(self.id_from_name(name))
	}
	
	#[inline]
	pub fn id_from_name(&self, name: &str) -> Id {
		*self
			.names
			.get(name)
			.expect(&format!("Item with name {} hasn't been registered", name))
	}
}
