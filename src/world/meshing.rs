...
pub mod registries {
    use crate::render::{BindGroupEntryConfig, Binding, BindingDescriptor, BufferConfig, Canvas, ComputeBatchDescriptor, MipParams, SamplerConfig, Tex, TextureViewConfig};
    use crate::util::collection::Registry;
    use crate::world::meshing::BlockMeshTemplate;
    use glam::Vec3;
    use log::error;
    use std::sync::OnceLock;
    use wgpu::{AddressMode, BufferUsages, MipmapFilterMode, ShaderStages, Texture, TextureViewDimension};
...
impl BlockMeshTemplate {
    fn cuboid(min: Vec3, max: Vec3, translucent: [bool; 6]) -> [Self; 6] {
...
impl BlockModel {
    pub fn new(meshes: Vec<TemplatedMesh>) -> Self {
        let templates = registries::block_mesh_templates();
        let mut cull = [false; 6];

        for mesh in meshes.iter() {
            let template = templates.by_id(mesh.template_id);
    ...
    fn build_meshes(chunk: &Chunk) -> (Mesh<NormTexVertex>, Mesh<AlphaVertex>) {
            ...
            for y in 0..n {
                for z in 0..n {
                    ...
                    for temp_mesh in block.model().meshes.iter() {
                        let template = templates.by_id(temp_mesh.template_id);
        ...
        let block_mesh = temp_poses
            ...
            .map(|(temp_mesh, mut poses)| {
                ...
                merged.with_texture(temp_mesh.texture_id)
            })
            .collect::<Vec<_>>()
            .merge();
