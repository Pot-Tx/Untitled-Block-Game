...
pub struct WorldRenderer {
	...
	block_batch: RenderBatch<'static, NormTexVertex, IntTransInst>,
	
	occlusion_desc: RenderDescriptor<'static>,
	occlusion_batch: RenderBatch<'static, AlphaVertex, IntTransInst>,
}
...
impl System for WorldRenderer {
	...
	type ResQuery = (ResWrite<Option<Frame>>, ResRead<Camera>, ResRead<RenderedWorld>);
	...
	fn operate(
		...
		let mut regions = res.2.regions.values().collect::<Vec<_>>();
		regions.retain(|&region| res.1.frustum.is_aabb_inside(region.bound()));
		...
		if let Some(frame) = res.0 {
			...
			frame.render(&self.occlusion_desc, |mut pass| {
				self.occlusion_batch
					.begin(&mut pass);
				
				regions.iter().for_each(|&region| {
					self.occlusion_batch
						.draw(&mut pass, region);
				});
			})
		}
...
impl WorldRenderer {
	pub fn new() -> Self {
		...
		Self {
			...
			block_batch: RenderBatch::<NormTexVertex, IntTransInst>::new(
				&canvas,
				RenderBatchDescriptor {
					name: "block",
					bindings: vec![
						Some(bindings.by_name("block_texture_sampler")),
						Some(bindings.by_name("camera")),
					],
					shader_name: "block",
					translucent: false,
			...
			occlusion_batch: RenderBatch::<AlphaVertex, IntTransInst>::new(
				&canvas,
				RenderBatchDescriptor {
					name: "occlusion",
					bindings: vec![
